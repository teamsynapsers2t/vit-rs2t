      ...
      <div className="grid xl:grid-cols-3 gap-8 mb-12">
        <div className="xl:col-span-1 space-y-6">
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-[32px] p-6 shadow-xl h-[350px] flex flex-col">
             <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2"><PieChartIcon className="w-5 h-5 text-teal-400" /> Capital Distribution</h3>
             <div className="flex-1 w-full relative overflow-y-auto pr-2 space-y-5 custom-scrollbar">
                  ...
                  return (
                    <div key={idx} className="w-full">
                      <div className="flex justify-between text-xs font-bold text-white mb-2">
                        <span>{item.name}</span>
                        <span style={{ color: item.color }}>₹{item.value.toLocaleString()} Cr ({pct.toFixed(1)}%)</span>
                      </div>
                      <div className="h-2 w-full bg-slate-900 rounded-full overflow-hidden">
                        <div className="h-full rounded-full transition-all duration-1000" style={{ width: `${pct}%`, backgroundColor: item.color }}></div>
                      </div>
                    </div>
                  );
                }) : (
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-[32px] p-8 shadow-xl mb-12">
            ...
            <div className="w-full space-y-6">
              {barData.map((item, idx) => {
                const maxVal = Math.max(...barData.map(d => Math.max(d.Actual, d.Optimal)));
                const actPct = maxVal > 0 ? (item.Actual / maxVal) * 100 : 0;
                const optPct = maxVal > 0 ? (item.Optimal / maxVal) * 100 : 0;
                return (
                  <div key={idx} className="w-full">
                    <div className="flex justify-between items-center text-sm font-bold text-white mb-2">
                       <span>{item.name}</span>
                       <div className="flex gap-4 text-xs font-mono">
                         <span className="text-sky-400">Actual: ₹{item.Actual.toLocaleString()}</span>
                         <span className="text-teal-400">Optimal: ₹{item.Optimal.toLocaleString()}</span>
                       </div>
                    </div>
                    <div className="space-y-1.5">
                      {/* Actual Bar */}
                      <div className="h-3 w-full bg-slate-900 rounded-full overflow-hidden flex items-center group relative">
                        <div className="h-full bg-sky-500 rounded-full transition-all duration-1000" style={{ width: `${actPct}%` }}></div>
                      </div>
                      {/* Optimal Bar */}
                      <div className="h-3 w-full bg-slate-900 rounded-full overflow-hidden flex items-center">
                        <div className="h-full bg-teal-500 rounded-full transition-all duration-1000 opacity-60" style={{ width: `${optPct}%` }}></div>
                      </div>
                    </div>
                  </div>
                );
